---
title: "Stripe MCP Server: Payment Data in Your AI Workflow"
description: "Set up the Stripe MCP server to query customers, subscriptions, and payment data in Claude or Cursor. Step-by-step guide with API key scoping and security tips."
...
Stripe leads MCPFind's finance category with 1,395 GitHub stars across 41 indexed finance servers. The gap between Stripe and the next closest server in the category is significant, and it reflects genuine adoption from developer teams who need payment data in their AI workflows. Instead of switching tabs to pull a customer's subscription status or dig through dispute history, you ask Claude directly. This guide covers what the Stripe MCP server exposes, how to install it in Claude Desktop or Cursor, how to scope API keys correctly, and which payment operations you can actually run through it. We also cover security practices for keeping production Stripe credentials safe when an AI agent holds them.
