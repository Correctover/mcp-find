...
The Stripe MCP server gives Claude, Cursor, and other AI tools direct access to your Stripe account: customers, subscriptions, invoices, disputes, and balance through plain-language prompts. The security setup matters as much as the config. A restricted API key with minimum permissions is the baseline. Prompt injection through malicious page content is a real vector when the AI also browses the web. PCI scope extends to any system where your live secret key sits at rest, including your MCP config file. This guide covers installation, API key scoping, OAuth considerations, and the audit steps most setup guides skip.
