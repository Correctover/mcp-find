---
title: "Playwright MCP Server: Why Accessibility-Tree Automation Beats Screenshot-Based Approaches"
description: "Microsoft's Playwright MCP server uses the browser accessibility tree, not screenshots, to give AI agents 25+ automation tools. Compare it to browser-use, Puppeteer MCPs, and direct Playwright code."
...
draft: false
cornerstone: false
howToName: "How to Install and Configure the Playwright MCP Server"
howToDescription: "Add Microsoft's Playwright MCP server to Claude Desktop or Cursor in three steps. Requires Node.js."
howToTotalTime: "PT5M"
howToSteps:
  - name: "Add Playwright MCP to your config file"
    text: "Open your claude_desktop_config.json (macOS: ~/Library/Application Support/Claude/) and add a mcpServers block with command: npx and args: [@playwright/mcp@latest]. For Cursor, open Settings > MCP Servers and paste the same block."
  - name: "Configure browser mode"
    text: "For headed mode (browser window visible during execution), add --headed to the args array. For a specific browser engine, add --browser and firefox or --browser and webkit as additional args."
  - name: "Verify the connection"
    text: "Open Claude Desktop or Cursor and ask it to take a browser snapshot of a URL. A successful response returns the page's accessibility tree. If no response arrives, check that Node.js is installed and restart the client."
faqItems:
...
Microsoft's Playwright MCP server connects AI agents to a real browser through the accessibility tree, not screenshots or vision models. The tree returns structured data about every interactive element, making clicks, form fills, and navigation reliable without inference. You get 25+ tools and support across Claude Desktop, Cursor, Windsurf, and GitHub Copilot. This guide covers the mechanism, installation, a full tool reference, and how Playwright MCP compares to browser-use, Puppeteer-based MCPs, and direct Playwright code.
...
These use cases work best when paired with a running local dev server. Playwright MCP is not a replacement for your CI test suite but a fast exploration and debugging tool that complements it. For a curated list of the top devtools servers that pair with testing and browser automation workflows, see [the best devtools MCP servers for code review](/blog/best-devtools-mcp-servers-code-review).
