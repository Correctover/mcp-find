---
title: "Best MCP Servers for Cloud Infrastructure (AWS, GCP, Cloudflare) — 2026"
description: "119 cloud MCP servers ranked by GitHub stars. Control AWS, GCP, and Cloudflare from Claude — deploy infra, query resources, and automate ops without leaving chat."
...
For a curated list of top servers in this space, see [the best MCP servers for DevOps and CI/CD](/blog/best-mcp-servers-devops-cicd).

## What Makes Cloud MCP Servers Different From Other Categories?
...
Read-only servers are the right default for shared environments. Granting write access through an MCP server requires the same approval and rollback process you would apply to any deployment automation. Browse the [cloud category on MCPFind](/categories/cloud) with the star filter set to 100+ to see the options that have already proven adoption in production environments. For teams building AI agent pipelines on top of cloud infrastructure, see [the best AI and machine learning MCP servers for agent toolchains](/blog/best-ai-ml-mcp-servers-agent-toolchains) for the servers that complement cloud automation most effectively.
